...
# 2026-09-26: S3 asks for 0 kg into the stopped screw (rulings 2026-09-26 W2.1;
# it allowed < 1.0 kg, which tick phase crossed), and LineFlow is off frame time.
# test_wash_line_timing (2026-09-26): 3B's VSS dosing screw meters its wash line
# (19 kg/h per rpm, the HMI setpoint, 50 rpm on a new line), the line holds
# 3 min to the extruder silo (the tanks most, the flotation tank 2/3), a stopped
# machine keeps what it holds, a full VSS runs the line 15 min, a rebuild keeps
# it all, 3A unchanged (operator rulings 2026-09-26 W2-W5, PLACEHOLDER numbers).
# 13 checks. docs/audit/wash_line_timing_2026-09-26.md.
# test_rebuild_pipe_carry (2026-09-25): LineFlow.rebuild() keeps the kg riding
# the connectors. rebuild() carried each surviving node by `id @ scene path`,
# but _link() cleared _edges and _init_pipes() built empty pipes, so every
# rebuild (a flow placement, deletion or jog, a line macro beside a running
# line, the line coupler) deleted them: 13.35 kg on a 3B line fed 60 s at
# 950 kg/h, the ledger 13.35 kg off for good. Now a loaded edge with both ends
# left keeps its stages and stage_t (an empty edge restarts at phase 0); a gone
# edge's kg go into the source's out batch, or the target's in batch when only
# the target is left. A real 3B line: a rebuild with nothing changed, line 3A
# placed beside it, a machine jogged, one deleted, two adjacent deleted (only
