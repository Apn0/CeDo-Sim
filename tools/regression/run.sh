...
# 950 kg/h, the ledger 13.35 kg off for good. Now an edge with both ends left
# keeps its stages and stage_t; a gone edge's kg go into the source's out
# batch, or the target's in batch when only the target is left. A real 3B
# line: a rebuild with nothing changed, line 3A placed beside it, a machine
# jogged, one deleted, two adjacent deleted (only the connector between them
# is lost), no "Cannot get path" engine error after a BuildMode-style delete.
# 25 checks; 6 mutations red, and main's LineFlow red. docs/audit/rebuild_pipe_carry_2026-09-25.md.
