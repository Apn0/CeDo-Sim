extends Node3D
## LINE 3A LEDGER — mass conservation, on a REAL MainWorld boot.
##
##   godot --headless --path <proj> res://src/tests/test_line3a_identity.tscn
##
...
## them proved conservation. This file closes that gap for line_3a, mirroring
...
##
## line_3a has no Line3ADef.gd (unlike line_3c/Line3CDef.gd) — no per-stage HMI
...
const TOUCHED := [
	...
	"user://__l3aident___save.json", "user://__l3aident___factory.json",
]
const DUMP_PATH := "user://line3a_identity.json"
...
func _ready() -> void:
	print("=== LINE 3A LEDGER — mass conservation (measured) ===")
...
func _build_world(bm, lf) -> void:
	...
	# count against LINE_3A_SEQ.size() — a handful of SEQ entries are furniture
	...
	_ok(listed.size() > 0, "the macro really built: %d LineFlow machines (of %d SEQ entries)"
		% [listed.size(), BuildMode.LINE_3A_SEQ.size()])
...
func _assert_ledger(lf) -> void:
	_section("LEDGER — mass conservation on line_3a")
	...
	_ok(injected > 0.0 and absf(balanced) <= tol,
		"mass ledger balances on line_3a: %.6f kg residual over %.2f kg charged (<= %.4f kg)"
