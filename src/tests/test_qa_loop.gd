...
# Building frame (bf) — the same fitted frame test_lump_cart_coverage.gd uses.
...
func _ready() -> void:
	...
	if bm == null:
		...
		_finish(world)
		return
	# bf(0,0) is the building's corner: half the line stood outside (29/37 under
	# the roof in the fitted frame). bf(4,22) is 37/37 (measured 2026-09-25).
	await _place_macro(bm, "line_3c", Vector2(4.0, 22.0))
