...
func _build_model() -> void:
	...
	# ── 6-Slot Walkie-Talkie Battery Charging Station ─────────────────────────
	var charger_base := StandardMaterial3D.new()
	charger_base.albedo_color = Color(0.12, 0.13, 0.14)
	charger_base.roughness = 0.5
	_box(Vector3(0.32, 0.05, 0.18), Vector3(-0.52, 0.79, 0.0), charger_base) # charger dock base

	var batt_mat := StandardMaterial3D.new()
	batt_mat.albedo_color = Color(0.25, 0.26, 0.28)
	batt_mat.roughness = 0.6

	var led_green := StandardMaterial3D.new()
	led_green.albedo_color = Color(0.15, 0.95, 0.25)
	led_green.emission_enabled = true
	led_green.emission = Color(0.15, 0.95, 0.25)
	led_green.emission_energy_multiplier = 2.5

	var led_red := StandardMaterial3D.new()
	led_red.albedo_color = Color(0.95, 0.20, 0.15)
	led_red.emission_enabled = true
	led_red.emission = Color(0.95, 0.20, 0.15)
	led_red.emission_energy_multiplier = 2.5

	# 6 battery slots (2 rows of 3)
	for row in range(2):
		for col in range(3):
			var bx : float = -0.52 - 0.08 + col * 0.08
			var bz : float = -0.04 + row * 0.08
			_box(Vector3(0.06, 0.07, 0.05), Vector3(bx, 0.83, bz), batt_mat) # battery pack
			var is_charged : bool = (row * 3 + col) < 4
			var led_mat := led_green if is_charged else led_red
			_box(Vector3(0.015, 0.015, 0.01), Vector3(bx, 0.82, bz + 0.035), led_mat) # status diode
