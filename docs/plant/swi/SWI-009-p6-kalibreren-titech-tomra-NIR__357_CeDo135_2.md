...
| 17 | Lampen aanzetten / inloggen (Switch lamps on / log in) | Functie (hand); Zicht (eye); TIP/info (blue i) | "Log in het menu in als 'operator' met wachtwoord '0000'." (Log into the menu as **'operator'** with password **'0000'**.) | Photo of HMI login screen: tabs "…ortering / Statistieken", prompt "Raak aan om in te loggen" (Touch to log in). |
...
- **HMI login:** user **'operator'**, password **'0000'**.
