...
> - runs the four stdlib gates `run.sh` exits on BEFORE any suite (seconds, no
>   Godot): `python3 tools/regression/lint_unused_params.py src`,
>   `python3 tools/hmi/palette_census.py`,
>   `python3 tools/audit/symbol_flow.py --root src --all`,
>   `python3 tools/audit/material_census.py --root src`. (Added 2026-09-26: the
>   resume merged without the census, and `main`'s full harness stopped at
>   `== material census ==` from `dde61eb` to `7e37741`;
...
| `docs/audit/lineflow_set_process_2026-09-26.md` | **21 suites that tick their own LineFlow now switch its `_process` off after `add_child`.** Before, LineFlow also ticked on frame time in every awaited frame: one tick before the suite's own `rebuild()` (kept through it, e.g. the silo sensor clock in `_silo_state`), one idle tick between `rebuild()` and `start_line()` (every empty connector's phase), and one per mid-run await (433 extra in `test_wet_side_beds`). Every heavy frame's delta read 0.117–0.150 s, so one tick per awaited frame. Measured before ×2, after ×2 and `--fixed-fps 10` both ways, plus an instrumented baseline: check values moved in 5 suites, info lines in 4, and `test_sort_line_topology` moved with the frame pacing alone. `test_extruder_silo_feed_stop` was left on frame time (operator choice): its S3 passed only with exactly one frame tick (probe k=0..3: +1.12 / +0.96 / +1.15 / +1.20 kg against < 1.0); re-derived the same day (next row). Also: the `test_machine_sounds` S8 beep race, the material census red on `main` |
...
| `docs/DESIGN_physical_bulk_material_2026-09-26.md` | **The physical material model the operator wants as the end game (rulings R1/R3 in `audit/hmi_rpm_rate_2026-09-26.md`): research and design, nothing built.** How games and real-time sims do piles (a resting heightfield store plus active bodies, AGX Terrain's template), what Godot 4.7 offers (no GPU device headless, measured), a throwaway benchmark (a turning screw moves film and a stopped one does not; a surface-velocity screw moves nothing; Rapier 0.8.34 as shipped throws blobs out of the bin; Jolt at 10k blobs with default limits loses them through the floor), blob counts for the 3C doseersilo, the kg-ledger contract with LineFlow, the machine-by-machine plan and 10 operator questions |
...
| `docs/audit/wash_line_timing_2026-09-26.md` | **3B's wash line held nothing: every machine conveyed at 6 kg/s, the first kg reached the extruder silo 38.8 s after the VSS, a 150 kg VSS emptied in 15 s.** From the operator's rulings (`operator_rulings_2026-09-26.md` W): M11a meters the line (19 kg/h per rpm, the HMI setpoint, 50 rpm on a new line), plug flow 3 min to the silo (the tanks most, the flotation tank 2/3), VSS 237.5 kg, silo 100 % at 277.1 kg, and a VSS -> dosing screw DIRECT feed on 3A/3B, so a stopped screw takes 0 kg. `LineFlow.WASH_TIMING`; holds ride the machines' out-connectors. `test_wash_line_timing` (13), `test_extruder_silo_feed_stop` S3 = 0 kg and off frame time, the 74-suite ripple, the mutation matrix. Found: the MACHINES screen's "drive" row multiplies the meter's rate on top (kept, W6) |
| `docs/plant/operator_rulings_2026-09-26.md` | **The wash line's timing, 2026-09-26 (section W)**: a stopped dosing screw takes nothing; M11a's 50-100 is screw rpm, ~1900 kg/h at 100; the line holds ~3 min (his first "~10 min" did not fit his silo answers, he chose "the line holds less"); a full VSS 15 min, the silo 15-20 min of feed; the tanks hold most, the flotation tank 2/3; 3A "differs", line 1 and 3C later. Recollections (CLAIMED) beside what the sim measured |
...
| `docs/research_film_physics_feasibility.md` | Film-physics R&D (2026-07-15) — GO on Jolt + GPUParticles3D + custom buoyancy; NO-GO on a Rapier backend swap. **Stale on the engine:** the game has run Rapier3D since 2026-07-22 (`project.godot:419`); see `DESIGN_physical_bulk_material_2026-09-26.md` §3.4 |
...
- **LineFlow ticks at 10 Hz, not per frame (since 2026-09-23).** Its
  ...
  **A suite that drives `tick()` itself must call `set_process(false)` AFTER
  ...
  Since 2026-09-26 the 21 `run.sh` suites that make and tick their own
  LineFlow switch it off there. Before, each got a frame tick before its own
  `rebuild()`, one between `rebuild()` and `start_line()`, and one per
  mid-run await (433 in `test_wet_side_beds`). Check values moved in 5
  suites. `test_extruder_silo_feed_stop` was left on frame time, because its
  S3 passed only on its one frame tick; since the same day's wash-line
  timing it is off too, and S3 asks for 0 kg into the stopped dosing screw
  (probe k=0..3 all PASS; `docs/audit/wash_line_timing_2026-09-26.md`).
  `docs/audit/lineflow_set_process_2026-09-26.md`.
- **A throughput limit is not a residence time.** Until 2026-09-26 every wash
  machine conveyed at MachineFlow's 6 kg/s (21 600 kg/h) and passed its input on
  within a tick, so 3B's line held 9.6 kg at 950 kg/h, the first kg reached the
  extruder silo 38.8 s after the VSS, and a 150 kg VSS emptied in 15 s, where
  the operator remembers minutes. Anything built on those times (a suite that
  waits for kg at the silo, a stop's overshoot) measured the model's shortcut.
  3B now meters at its dosing screw and holds 3 min (`LineFlow.WASH_TIMING`,
  holds on the out-connectors, advancing with the machine's spin); 3A, line 1
  and 3C still pass through in seconds. A suite that feeds 3B and expects kg
  downstream must wait `wash_timing("line_3b").first_kg_s` first (as
  `test_rebuild_pipe_carry` now does; with its old 60 s only 3 connectors were
  loaded). `docs/audit/wash_line_timing_2026-09-26.md`.
...
- **A by-name compare only guards what the fixture carries.** Until 2026-09-26
  every batch the suite fed was clean (`MaterialBatch.new(…, 0.0, 0.0)`), so a
  restore that dropped `contaminant_kg` passed 62 of 62. Phase A now feeds line 1
  an Alba Marl bale sample and A11 sums water and contaminant per body in LineFlow
  itself. When you add a quantity to a save, make the fixture carry it first.
...
- each extruder silo has a laser level sensor (`LineFlow._tick_silo_feed_stops`)
  ...
  (3B's silo: 277.1 kg since 2026-09-26, `LineFlow.WASH_TIMING`);
