...
- **LineFlow ticks at 10 Hz, not per frame (since 2026-09-23).** Its
  ...
  **A suite that drives `tick()` itself must call `set_process(false)` AFTER
  ...
  suites. Left on frame time on purpose: `test_extruder_silo_feed_stop`,
  whose S3 passes only on its one frame tick, until S3 is re-derived.
